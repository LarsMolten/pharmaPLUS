...
function liste_codedentaire() {

  $("#libellecodedentaire").val("");
  iddente = ""; 

  $("#ajoutercodedentaire").text("Ajouter");
  $.ajax({
    beforeSend: function () {
      $(".all-contacts").block({
        message:
    ...
    url: base + "liste_codedentaire",
    type: "POST",
    error: function(xhr, status, error) {
       alertCustom("danger", 'ft-x', "Une erreur s'est produite");
    } ,success: function (response) {
      $("#table_codedentaire ").empty();
      $("#table_codedentaire").append(response);
     
      // Convertissez la chaîne JSON en objet JavaScript
        $("#table_codedentaire").DataTable({
        destroy: true,
        ordering: true,
        order: [[0, "desc"]],
        info: false,
...
function editcodedentaire(ids) {


    var nom = $("#codedentaire_" + ids).data("libelle");

    $('#libellecodedentaire').val(nom);
    $("#ajoutercodedentaire").text("Modifier");
...
$("#ajout_codedentaire").off("submit").on("submit",function (e) {
  e.preventDefault();
  var formData = new FormData(this);

  formData.append("id",iddente);
...
function close_overlay_liste_codedentaire() {
  $(".all-contacts").unblock();
}

// Fonction pour dialogue une catégorie  ************************************************
function supprimercodedentaire(id) {
  $(".all-contacts").block({
    message:
      `
    ...
    <div class="card-content">
        <div class="card-body">
            <p>Confirmez-vous cette suppression ?</p>
                ...
                <button type="button" onclick="delete_codedentaire_from_dialog(` +
      id +
      `)" class="mr-1 mb-1 btn btn-sm btn-warning btn-min-width"><i class="ft-check"></i> Oui</button>
                <button type="button" onclick="close_overlay_liste_codedentaire()" class="mr-1 mb-1 btn btn-sm btn-outline-light btn-min-width"><i class="ft-x"></i> Annuler</button>
...
function delete_codedentaire_from_dialog(id) {
  $.ajax({
    beforeSend: function () {
      $(".all-contacts").block({
        message:
    ...
    url: base + "supprimer_codedentaire",
    type: "POST",
    data: { id: id },
    error: function(xhr, status, error) {
       alertCustom("danger", 'ft-x', "Une erreur s'est produite");
